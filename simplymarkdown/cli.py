...
from simplymarkdown.config import DEFAULT_FAVICON, DEFAULT_TEMPLATE, DEFAULT_THEME, BuildConfig, Config, RSSConfig
...
def build(
    ...
    config = Config(
        ...
        title=title,
        root_url=root_url,
        rss=RSSConfig(whitelist=rss_whitelist, description=rss_description),
    ...
    if watch_mode or serve_mode:
        if serve_mode and watch_mode:
            serve_with_watch(input_path, output_path, do_build, host=host, port=port, open_browser=not no_open)
        elif serve_mode:
...
def init(project_dir: str, title: str, root_url: str, favicon: str) -> None:
    ...
    for dir_path in ["source", "source/posts", "source/modules", "source/static/img", "source/static/css", "output"]:
        (project_path / dir_path).mkdir(parents=True, exist_ok=True)

    config = Config(input_dir="source", output_dir="output", title=title, root_url=root_url, favicon=favicon)
    config.to_yaml(project_path / "simplymarkdown.yaml")

    (project_path / "source" / "index.md").write_text(f"""---
title: {title}
---
...
""")
    ...
    today = datetime.now()
    posts_dir = project_path / "source" / "posts" / today.strftime("%Y/%m/%d")
    posts_dir.mkdir(parents=True, exist_ok=True)
    (posts_dir / "my-first-post.md").write_text(f"""---
title: My First Post
...
def new(title: str, project_dir: str) -> None:
    ...
    project_path = Path(project_dir).resolve()
    source_dir = project_path / "source" if (project_path / "source").exists() else project_path
